...
use App\Settings\FallbackGeneralSettings;
...
if (!function_exists('safe_currency_symbol')) {
    function safe_currency_symbol($fallback = 'TK')
    {
        $symbol = trim((string) (gs()->currency_symbol ?? ''));

        if ($symbol === '' || $symbol === '?' || $symbol === 'Î±Âºâ”‚' || $symbol === 'à§³') {
            return $fallback;
        }
...
if (!function_exists('price')) {
    function price($data, $decimals = 2)
    {
        ...
        return safe_currency_symbol() . $formatted;
    }
}
...
if (!function_exists('gs')) {
    function gs()
    {
        ...
        try {
            ...
            $settings = new FallbackGeneralSettings();
        }
...
if (!function_exists('get_image')) {
    function get_image($path)
    {
        ...
        if ($path === '') {
            $logo = trim((string) (gs()->logo ?? ''));

            if ($logo !== '') {
                return get_image($logo);
            }

            return 'https://ui-avatars.com/api/?name=MS2BD&size=512&background=1f2937&color=ffffff';
        }
...
if (!function_exists('get_product_image')) {
    function get_product_image($product)
    {
        ...
        $path = ltrim($path, '/');
        $relativePath = str_starts_with($path, 'uploads/') ? $path : ('uploads/' . $path);

        if (file_exists(public_path($relativePath))) {
            return asset($relativePath);
        }

        $sourceSiteUrl = rtrim((string) ($product->source_site_url ?? ''), '/');
        $localUrl = rtrim((string) config('app.url'), '/');

        if ($sourceSiteUrl !== '' && strcasecmp($sourceSiteUrl, $localUrl) !== 0) {
            return $sourceSiteUrl . '/' . $relativePath;
        }
