    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('orderid')) {
            ...
            if ($status === 'success') {
                ...
                $order->save();
                $this->syncAutoTopupTracking($order, $status, $content, $request->all());
        ...
        $this->syncAutoTopupTracking($order, $status ?? '', $message, $data);

        return response()->json(['message' => 'Webhook processed successfully']);
    }

    private function syncAutoTopupTracking(Order $order, string $status, string $content, array $payload = []): void
    {
        $status = strtolower(trim($status));

        $forwardStatus = match ($status) {
            'success', 'complete', 'completed', 'finish' => 'completed',
            'failed', 'cancel', 'cancelled', 'canceled', 'error' => 'failed',
            default => 'processing',
        };

        AutoTopupOrder::updateOrCreate(
            ['order_id' => $order->id],
            [
                'provider' => 'vnbazer',
                ...
                'failure_reason' => $forwardStatus === 'failed' ? $content : null,
            ]
        );
    }
