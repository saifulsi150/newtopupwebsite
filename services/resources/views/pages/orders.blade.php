        ...
        <div class="orders-list border-b-2 m-2">
          <div class="sm:flex">
            ...
            <div class="w-full sm:w-1/2">
              ...
              {{-- detect apply হলে user account info hide থাকবে --}}
              @php
                  $detectApplied = !empty($order->error_message)
                      && str_contains(strtolower((string) $order->error_message), 'auto-detect');
                  $showAccountInfo = !empty($order->account_info) && !$detectApplied;
                  $normalizedStatus = \App\Constants\OrderStatus::normalize($order->status);
                  $showPlayerId = in_array($normalizedStatus, [\App\Constants\OrderStatus::COMPLETE, \App\Constants\OrderStatus::CANCEL], true);

                  $detectInfo = [];
                  if ($detectApplied && !empty($order->account_info)) {
                      $rawDetectInfo = $order->account_info;
                      if (is_string($rawDetectInfo)) {
                          $decodedDetectInfo = json_decode($rawDetectInfo, true);
                          $detectInfo = is_array($decodedDetectInfo) ? $decodedDetectInfo : [];
                      } elseif (is_object($rawDetectInfo)) {
                          $detectInfo = (array) $rawDetectInfo;
                      } elseif (is_array($rawDetectInfo)) {
                          $detectInfo = $rawDetectInfo;
                      }
                  }

                  $detectMessage = trim((string) ($detectInfo['player_id'] ?? ''));
              @endphp

              @if ($detectApplied && $showPlayerId)
                  @if ($detectMessage !== '')
                      <p class="px-3 py-1 text-left">
                          <span class="font-bold">Player ID: </span> {{ $detectMessage }}
                      </p>
                  @else
                      <p class="px-3 py-1 text-left text-gray-500">
                          <span class="font-bold">Account Info:</span> Hidden for verification.
                      </p>
                  @endif
              @endif

              @if ($showAccountInfo && !empty($order->account_info))
                  @php
                      $rawInfo = $order->account_info;
                      if (is_string($rawInfo)) {
                          $accountInfo = json_decode($rawInfo, true);
                      } elseif (is_object($rawInfo)) {
                          $accountInfo = (array) $rawInfo;
                  ...
                  @if (!empty($accountInfo))
                      @if (!empty($accountInfo['player_id']) && $showPlayerId)
                          <p class="px-3 py-1 text-left">
                              <span class="font-bold">Player ID: </span> {{ $accountInfo['player_id'] }}
                          </p>
                      @endif
                      @if (!empty($accountInfo['account_type']))
                          <p class="px-3 py-1 text-left">
                              <span class="font-bold">Account Type: </span> {{ $accountInfo['account_type'] }}
                          </p>
                      @endif
                      @if (!empty($accountInfo['game_account']))
                          <p class="px-3 py-1 text-left">
                              <span class="font-bold">Game Account: </span> {{ $accountInfo['game_account'] }}
                          </p>
                      @endif
                      @if (!empty($accountInfo['subscription_details']))
                          <p class="px-3 py-1 text-left">
                              <span class="font-bold">Subscription Details: </span> {{ $accountInfo['subscription_details'] }}
                          </p>
                      @endif
                  @endif
              @endif
              ...
              <p class="px-3 py-1 text-left">
                ...
                <span class="{{ \App\Constants\OrderStatus::color($order->status) }}">
                                    <span class="order-status">{{ \App\Constants\OrderStatus::text($order->status) }}</span>
                </span>
              </p>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="{{ asset('page/contact-us') }}" type="button" class="btn theme-btn">Contact Us</a>
