...
<div>
   <div class="p-2 container m-auto checkout_page">
      <div class="bg-white border rounded-md">
         <div class="flex">
            <div>
               <img class="rounded-3xl p-2 w-24 h-24" src="{{ get_product_image($product) }}" alt="{{ $product->title }}" fetchpriority="high" decoding="async">
            </div>
      ...
      <div>
         <form method="POST" action="{{ route('topup.buynow') }}" class="md:flex gap-2" autocomplete="off">
           @csrf
              ...
              @if (!$product->isVoucher())
               ...
               <section>
                  <div class="border bg-white rounded-md">
                     ...
                     <hr>
                     @php($hasDynamicFields = is_array($product->dynamic_fields) && count($product->dynamic_fields) > 0)
                     <!-- Dynamic Fields Component -->
                     <x-checkout.dynamic-fields-form :product="$product" />
                        ...
                        <div class="col-md-12 text-left px-3">
                           <div>
                              ...
                              <p class="fb-normal text-xs flex items-center mb-3" style="color: gray;">
                                 ...
                                 <span id="total_cost" style="display:none;">0</span>
                              </p>
                             @auth
                              <div>
...
<script>
    $(document).ready(function() {
        ...
        const $headerWalletBalance = $('#header_wallet_balance');
      ...
      const $orderForm = $('form[action="{{ route('topup.buynow') }}"]');
      const balanceRefreshUrl = "{{ route('account.balance') }}";
      ...
      function refreshLiveBalance() {
         ...
         .then(function(data) {
            ...
            const balanceText = String(data.balance_text ?? data.balance ?? '0');
            $walletBalance.text(balanceText);

            if ($headerWalletBalance.length) {
               $headerWalletBalance.text(balanceText + '৳');
            }
         ...
         .catch(function() {
            // Silent fail: keep existing rendered balance if refresh request fails.
         });
      }

      function clearVariationUI() {
      ...
      function resetTopupState() {
         ...
         // Clear all user-entered form values for a fresh checkout attempt.
         $orderForm.find('input[type="text"], input[type="email"], input[type="number"], input[type="password"], textarea').val('');
         $orderForm.find('select').each(function() {
            this.selectedIndex = 0;
         });

         isSubmittingOrder = false;
         $buyNow.prop('disabled', true).text('Buy Now');
         $addFund.hide();
      ...
      // Prevent multi-click / rapid re-submit from creating duplicate requests.
      $orderForm.on('submit', function() {
         ...
         sessionStorage.setItem('topup_last_order_at', String(Date.now()));
         $buyNow.prop('disabled', true).text('Processing...');
         return true;
      });
        ...
        function checkWallet() {
         ...
         const walletBalance = parseNumericBalance($walletBalance.text());
        ...
        function autoSelectPaymentMethod(cost) {
         if (parseNumericBalance($walletBalance.text()) < cost) {
                $('#payment_gateway').click();
            }

         if ($('#payment_method').val() == "wallet" && cost > parseNumericBalance($walletBalance.text())) {
                disableBuyNow();
            } else {
                enableBuyNow();
            }
        ...
        function initializePaymentMethod() {
            ...
            if (parseNumericBalance($walletBalance.text()) > 0) {
                    $wallet.click();
                } else {
                    $('#payment_gateway').click();
                }
      ...
      // If page is restored from browser cache or returned via back button after ordering,
      // force a clean form so previous account/package data is not shown.
      window.addEventListener('pageshow', function(event) {
         const hasRecentOrderFlag = !!sessionStorage.getItem('topup_last_order_at');
         if (event.persisted || hasRecentOrderFlag) {
            resetTopupState();
            sessionStorage.removeItem('topup_last_order_at');
            refreshLiveBalance();
         }
      });

      const hasRecentOrderFlag = !!sessionStorage.getItem('topup_last_order_at');
      if (hasRecentOrderFlag) {
         resetTopupState();
         sessionStorage.removeItem('topup_last_order_at');
         refreshLiveBalance();
      }
      ...
      // Prevent accidental horizontal swipe/slide on mobile topup page.
      let touchStartX = 0;
      let touchStartY = 0;

      document.addEventListener('touchstart', function(e) {
         if (window.innerWidth > 768 || !e.touches || !e.touches.length) {
            return;
         }
