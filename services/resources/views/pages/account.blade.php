  ...
  <div class="p-2">
    ...
    <div class="text-center w-full flex mb-3 md:mb-8 justify-center items-center">
      <span class="capitalize primary-font-color-text font-16">
        <b class="font-bold">Available Balance : {{ safe_currency_symbol() }}{{ amount(Auth::user()->balance) }}</b>
      </span>
    ...
    <div style="max-width: 700px; margin: auto;">
      <div class="text-center grid md:grid-cols-4 grid-cols-2 md:gap-4 gap-3 my-2 md:my-5 mb-10 statics-container">
        <div class="bg-white statics">
          <h2 class="text-lg font-normal fb-normal statics-heading">{{ safe_currency_symbol() }}{{ amount(Auth::user()->balance) }}</h2>
        ...
        <div class="bg-white statics">
          <h2 class="text-lg font-normal fb-normall statics-heading">{{ safe_currency_symbol() }}0</h2>
      ...
      <div class="w-full text-left bg-white my-4 account-info-container" style="padding: 14px 16px;">
        <a href="{{ route('notification.settings') }}" class="d-flex justify-content-between align-items-center" style="text-decoration:none; color: inherit;">
          <div>
            <div class="text-lg primary-font-color-text font-normal fb">Notification Settings</div>
            <div style="font-size: 13px; color: #64748b;">Enable browser notifications from one switch.</div>
          </div>
          <div style="font-size: 20px; font-weight: 700; color: #0f172a;">></div>
        </a>
      </div>
    </div>
  </div>
